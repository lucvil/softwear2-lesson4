...
int main (int argc, char**argv)
{
  ...
  if (argc != 3){
    fprintf(stderr, "usage: %s <binary file of item information> <max capacity (double)>\n",argv[0]);
    exit(1);
  }
...
Itemset *load_itemset(char *filename){
    ...
    fread(itemvalue,sizeof(double),*number,fp);

    double itemweight[*number];
    fread(itemweight,sizeof(double),*number,fp);
    ...
    for(int i = 0; i < *number; i++){
        item[i].value = itemvalue[i];
        item[i].weight = itemweight[i];
    }
    *list = (Itemset){.number = *number, .item = item};
